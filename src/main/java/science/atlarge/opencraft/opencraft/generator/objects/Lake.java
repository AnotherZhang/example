        ...
        for (int x = 0; x < (int) MAX_DIAMETER; x++) {
            for (int z = 0; z < (int) MAX_DIAMETER; z++) {
                for (int y = 0; y < (int) MAX_HEIGHT; y++) {
                    ...
                    GlowBlock block = (GlowBlock) world.getBlockAt(sourceX + x, sourceY + y, sourceZ + z);
        ...
        for (int x = 0; x < (int) MAX_DIAMETER; x++) {
            for (int z = 0; z < (int) MAX_DIAMETER; z++) {
                for (int y = (int) MAX_HEIGHT / 2; y < (int) MAX_HEIGHT; y++) {
                    ...
                    GlowBlock block = (GlowBlock) world.getBlockAt(sourceX + x, sourceY + y - 1, sourceZ + z);
                    Block blockAbove = block.getRelative(BlockFace.UP);
                    if (block.getType() == Material.DIRT
                            && !blockAbove.getType().isOccluding()
                            && blockAbove.getLightLevel() > 0) {
                        block.setTypeNoBroadcast(mycelBiome ? Material.MYCEL : Material.GRASS);
                    }
                }
            }
        }
