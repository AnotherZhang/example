...
public class BlockStateDelegate {
    ...
    /**
     * Roll-back previously backed-up block states, but doesn't broadcast updates to players.
     */
    public void rollbackBlockStatesNoBroadcast() {
        for (BlockState state : blockStateBackupMap.values()) {
            ((GlowBlockState) state).updateNoBroadcast(true);
        }
        blockStateBackupMap.clear();
    }
    ...
    /**
     * Updates all block states contained in the BlockState list, without broadcasting the updates to players.
     */
    public void updateBlockStatesNoBroadcast() {
        for (BlockState state : blockStateMap.values()) {
            ((GlowBlockState) state).updateNoBroadcast(true);
        }
        blockStateMap.clear();
