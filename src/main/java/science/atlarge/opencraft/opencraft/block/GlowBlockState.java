...
public class GlowBlockState implements BlockState {
    ...
    /**
     * Same as update(boolean force), but doesn't broadcast block updates to players
     */
    public final boolean updateNoBroadcast(boolean force) {
        GlowBlock block = getBlock();

        return (block.getTypeId() == typeId || force)
                && block.setTypeIdAndDataNoBroadcast(typeId, getRawData(), true);
    }
