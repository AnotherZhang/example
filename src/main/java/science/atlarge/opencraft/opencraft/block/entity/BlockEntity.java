...
public abstract class BlockEntity {
    ...
    public final void updateInRange() {
        IntCoordinates2D coordinates = new IntCoordinates2D(block.getX() >> 4, block.getZ() >> 4);
        block.getWorld().getRawPlayers().stream().filter(player -> player.canSeeChunk(coordinates))
                .forEach(this::update);
    }
